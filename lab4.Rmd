...
*Vladyslav Vasylchenko, Max Holovin, Ostap Kostiuk*
...
In this problem we have two independent samples $X_1,\dots,X_{100}$ and $Y_1,\dots,Y_{50}$, which we model as observations from normal distributions $X_i \sim N(\mu_1, 1)$ and $Y_j \sim N(\mu_2, 1)$.

We test the hypotheses $H_0: \mu_1 = \mu_2 \quad \text{vs} \quad H_1: \mu_1 \neq \mu_2$ under known variances $\sigma_1^2 = \sigma_2^2 = 1$.
...
$$ where $\Phi^{-1}$ is the quantile function of the standard normal distribution (the `qnorm` function in R).

Since the variances are known, the standard procedure is the two-sample Z-test for the difference of means.
...
We use a two-sided test because the alternative hypothesis is $H_1:\mu_1 \neq \mu_2$.

The significance level is $\alpha = 0.05$.

For the standard normal distribution the critical points are $z_{1-\alpha/2} = z_{0.975}$. The critical region is $$
...
-   If $p\text{-value} < 0.05$, we reject $H_0$ at the significance level $\alpha = 0.05$ and conclude that $\mu_1$ and $\mu_2$ are statistically different.

-   If $p\text{-value} \ge 0.05$, we do not reject $H_0$: the observed data do not provide sufficient evidence to claim that the population means are different.

# Problem 2

In this part we test hypotheses about the variances of two normal populations:
...
where $X_1,\dots,X_{n_1}$ and $Y_1,\dots,Y_{n_2}$ are the same samples as in Problem 1, with $n_1 = 100$, $n_2 = 50$, and the means $\mu_1$, $\mu_2$ are assumed to be unknown.

We assume that $$
X_i \sim N(\mu_1, \sigma_1^2), \qquad Y_j \sim N(\mu_2, \sigma_2^2),
$$ and that the samples are independent. The standard test for comparing two variances of normal distributions is the **F-test**.
...
Under the null hypothesis $H_0:\sigma_1^2 = \sigma_2^2$ the test statistic $$
F = \frac{S_1^2}{S_2^2}
$$ has the F-distribution with $(n_1-1, n_2-1)$ degrees of freedom: $$
F \sim F_{n_1-1,,n_2-1}.
$$

Since the alternative hypothesis is one-sided ($\sigma_1^2 > \sigma_2^2$), the critical region for significance level $\alpha = 0.05$ is $$
\mathrm{Cr} = { f : f > F_{1-\alpha}(n_1-1, n_2-1) },
$$ where $F_{1-\alpha}(n_1-1, n_2-1)$ is the $(1-\alpha)$-quantile of the $F_{n_1-1,,n_2-1}$ distribution.
...
If $F_obs \> F_crit$, we reject $H_0$ at the significance level $\alpha = 0.05$. Otherwise, we do not reject $H_0$.

For the one-sided test $H_1:\sigma_1^2 > \sigma_2^2$ the p-value is $$
p\text{-value}
= P\bigl(F_{n_1-1,,n_2-1} \ge F_{\text{obs}}\bigr)
= 1 - F_F(F_{\text{obs}}),
$$ where $F_F$ is the cumulative distribution function of the F-distribution.
...
• If $p\text{-value} < 0.05$, we reject $H_0$ at the significance level $\alpha = 0.05$ and conclude that the variance of the first population is significantly larger than the variance of the second population.

• If $p\text{-value} \ge 0.05$, we do not reject $H_0$. In this case, the observed data do not provide sufficient evidence to claim that $\sigma_1^2 > \sigma_2^2$.
...
b)  Task: Check if $\{|x_k|\}_{k=1}^{100}$ are exponentially distributed with $\lambda = 1$.\
...
c)  Task: Check if $\{x_k\}_{k=1}^{100}$ and $\{y_l\}_{l=1}^{50}$ have the same distributions.\
...
In this task you’ll practice fitting the regression line to some real-life features and analyzing the results. The file data.csv contains data on students, specifically their study time and corresponding marks. Your tasks are as follows: (a) Create a scatter plot of Marks vs. Study Time and provide brief comments;\
...
plot(data$time_study, data$Marks,
     ...
     ylab = "Marks",
     main = "Scatter Plot: Marks vs Study Time",
     pch = 19)
```

*Comments*: The trend is obvious - with the rise of study time, values of marks also increase. An approximately linear relationship is observed, which allows the application of linear regression
...
To find the regression equation, we look for a straight line that best describes the relationship between study time (X) and marks (Y). The line has the form:\
Y = a + bx,\
where a is an intercept and b is a slope.\
We get these values by using *least squares method*.

**(c)**

The model fits the data very well.\
The R-squared is 0.8878, which means that about 88.8% of the variation in marks is explained by study time.\
The Residual Standard Error (RSE) is 4.82, meaning that, on average, the predictions differ from the actual marks by about 4–5 points.\
For real-life data, this is reasonably accurate.\
The F-statistic is 775.8 with a p-value below 2e-16, which shows that the overall regression model is highly significant.\
In other words, the linear relationship is not due to some random chance.\
Overall, the regression line provides a very good fit to the data.

**(d)**

We test whether the slope (the coefficient of `time_study`) is equal to zero.

-   **H₀:** a = 0

    **H₁:** a != 0

This is a **t-test** on the slope coefficient.

So the test statistic is:

t = estimate / SE

From summary we saw earlier we already know these values, so let's plug them in.

t = 27.853.

As R shows - the p-value is extremely small - practically zero.

Since the p-value is far below 0.05, we **reject H₀**.

Study time **is highly significant** in predicting marks.

**(e)** Using the regression equation Y = a + bx, we can plug in x = 8.\
Y = 46.733858.

We can also do the same using R function:

```{r}
predict(model, newdata = data.frame(time_study = 8))
```

**(f)**

First thing we can do is to use more predictors (if we had them), such as for example *hours of sleep*, or *course difficulty*.\
Also, as we can see from the plot earlier our data has slight curvature, so allowing non-linear relationship should be a good idea.
